...
/** Accepts API/session JSON in camelCase or PascalCase. */
export function normalizeLoginResponse(raw: unknown): LoginResponse | null {
  ...
  if (typeof userIdRaw === 'number' && !Number.isNaN(userIdRaw)) userId = userIdRaw
...
export function requireLoginResponse(raw: unknown): LoginResponse {
  ...
  if (!n) throw new Error('Invalid login response')
  return n
}
