...
/** Parse API date (ISO date or datetime); stable for sorting */
...
/** REQ-023 → 223; used when multiple requests share the same calendar day */
...
/** Pipeline order: open work first when sorting ascending by status */
function statusRank(s: string): number {
  const x = s.trim().toLowerCase()
  if (x === 'registered') return 0
  if (x === 'in progress') return 1
  if (x === 'completed') return 2
  return 3
}

export function compareMaintenanceRequests(
  a: MaintenanceRequest,
  b: MaintenanceRequest,
