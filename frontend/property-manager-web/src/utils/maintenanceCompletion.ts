...
/** Count completed requests whose <code>dateCreated</code> falls in the chosen period (same basis as dashboard summary). */
export function countCompletedInPeriod(
  ...
  if (period === 'all') return completed.length
...
export function recentlyCompletedRequests(
  requests: MaintenanceRequest[],
  limit: number,
): MaintenanceRequest[] {
  return [...requests]
