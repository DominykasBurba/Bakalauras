...
/** Builds the same PDF used for download and view. */
export function createBillPdfDocument(b: Bill, options: BillPdfOptions): jsPDF {
  ...
  doc.text(
    options.kind === 'invoice' ? 'Bill / statement' : 'Payment receipt',
    margin,
    y,
  )
  y += 16
...
/**
 * PDF for a bill from the API: unpaid → invoice-style; paid → payment receipt.
 */
