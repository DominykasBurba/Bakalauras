...
import { isAdminRole, normalizeLoginResponse, SUPPRESS_LOGIN_RETURN_PATH_KEY } from '../utils/auth';
...
export function AuthProvider({ children }: {
    ...
    const logout = useCallback(() => {
        try {
            sessionStorage.setItem(SUPPRESS_LOGIN_RETURN_PATH_KEY, '1');
        }
        catch {
            void 0;
        }
    ...
    useEffect(() => {
        if (!auth?.token || isAdminRole(auth.role))
            return;
        if (auth.profileStatus !== undefined && auth.profileStatus !== null)
            return;
        queueMicrotask(() => {
            void refreshAuth();
        });
    }, [auth?.token, auth?.role, auth?.profileStatus, refreshAuth]);
    return (<AuthContext.Provider value={{ auth, login, logout, refreshAuth }}>
      {children}
