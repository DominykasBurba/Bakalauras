...
import { changePassword, getAuthSession } from '../services/api';
...
export function AccountSettingsPage() {
    ...
    const roomDisplay = formatRoomUnitLabel(session?.unit) || session?.unit?.trim() || null;
      ...
      <div className="account-settings-layout">
        <section className="card work-order-card account-profile-card">
          ...
          {!sessionLoading && session && (<>
              ...
              {resident && (<div className="profile-section">
        ...
        <section className="card work-order-card account-password-card">
          ...
          <label>
            Current password
            <input type="password" autoComplete="current-password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} disabled={saving} required/>
          </label>
          <label>
            New password <span className="muted small">(min. 8 characters)</span>
            <input type="password" autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} disabled={saving} required minLength={8}/>
          </label>
          <label>
            Confirm new password
            <input type="password" autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={saving} required/>
          </label>
