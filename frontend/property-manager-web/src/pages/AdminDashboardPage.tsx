...
import { getDashboardSummary, getMaintenanceRequests } from '../services/api';
...
export function AdminDashboardPage() {
    ...
    const latestMaintenance = useMemo(() => sortMaintenanceRequests(scopedRequests, 'dateCreated', 'desc').slice(0, 5), [scopedRequests]);
    const completedCount = useMemo(() => countCompletedInPeriod(scopedRequests, completedPeriod), [scopedRequests, completedPeriod]);
    const recentlyCompleted = useMemo(() => recentlyCompletedRequests(scopedRequests, 12), [scopedRequests]);
    const buildingLine = (r: MaintenanceRequest) => r.buildingName ??
        (r.buildingId == null
            ? '—'
    ...
    return (<div className="page admin-dashboard-page">
      <div className="stats-grid stats-grid-admin">
        ...
        <div className="stat-card">
          ...
          <label className="admin-dashboard-completed-filter">
            <select value={completedPeriod} onChange={(e) => setCompletedPeriod(e.target.value as CompletedPeriod)} aria-label="Filter completed count by period">
      ...
      <section className="card admin-activity-card">
        ...
        {loading ? (<p className="muted admin-activity-loading">Loading…</p>) : latestMaintenance.length === 0 ? (<p className="muted">No maintenance requests in this scope.</p>) : (<ul className="admin-activity-list">
            {latestMaintenance.map((r) => (<li key={r.id}>
      ...
      <section className="card admin-dashboard-completed-section">
        ...
        {loading ? (<p className="muted">Loading…</p>) : recentlyCompleted.length === 0 ? (<p className="muted">No completed requests in this scope.</p>) : (<ul className="completed-list">
            {recentlyCompleted.map((t) => (<li key={t.id}>
