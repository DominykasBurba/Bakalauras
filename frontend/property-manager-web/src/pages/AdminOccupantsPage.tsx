...
/** Date-only string for occupancy start (server <see cref="DateOnly"/>). */
...
export function AdminOccupantsPage() {
  ...
  const vacantCreateUnits = useMemo(
    () => createUnits.filter((u) => !createOccupiedUnitIds.has(u.id)),
    [createUnits, createOccupiedUnitIds],
  )

  const selectableEditUnits = useMemo(() => {
    if (!editRow) return []
    const cur = editRow.unitId
    return editUnits.filter(
      (u) => !editOccupiedByOthers.has(u.id) || (cur != null && u.id === cur),
    )
    ...
    Promise.all([getUnitsForBuilding(auth.token, bid), getOccupancies(auth.token, bid, true)])
      .then(([units, occ]) => {
        ...
        const occupied = new Set(
          active.filter((o) => o.userId !== residentId).map((o) => o.unitId),
        )
  ...
  }, [auth?.token, editRow?.id, editForm.buildingId, showToast])
  ...
  async function submitDecline() {
    if (!auth?.token || !declineModalUser) return
    ...
    setDeclineError('')
    setActionUserId(declineModalUser.id)
    try {
      await declineOccupantProfile(auth.token, declineModalUser.id, c)
  ...
  useEffect(() => {
    if (detailOpenForId == null || !auth?.token) return
    let cancelled = false
  ...
  async function submitEdit(e: FormEvent) {
    ...
    try {
      ...
      const nextUnitId =
        nextUnitRaw == null || Number.isNaN(nextUnitRaw) ? null : nextUnitRaw
      ...
      if (prevUnitId !== nextUnitId && nextUnitId == null && prevUnitId != null) {
        ...
        if (bOcc != null) {
          ...
          if (mine) await endOccupancy(auth.token, mine.id)
        }
      }
  ...
  async function confirmRemove() {
    if (!auth?.token || !removeUser) return
    setRemoveBusy(true)
    setListError('')
    try {
  ...
  return (
    <div className="page admin-occupants-page">
      <h1>Occupants</h1>
      ...
      <section className="card admin-occupants-create">
        ...
        <form className="admin-occupants-create-form" onSubmit={(e) => void handleCreate(e)}>
              ...
              <div className="admin-occupants-create-grid admin-occupants-create-grid--stack">
                <label className="admin-occupants-create-field">
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, buildingId: e.target.value, unitId: '' }))}
                    disabled={creating}
                  >
                ...
                <label className="admin-occupants-create-field">
                  ...
                  <select
                    ...
                    disabled={creating || form.buildingId === ''}
                  >
                    <option value="">— No unit yet —</option>
                    {vacantCreateUnits.map((u) => (
          ...
          {form.buildingId !== '' && createUnits.length === 0 ? (
            <p className="muted small admin-occupants-create-inline-hint">
              This building has no units yet. Add units under Property portfolio before assigning a room.
            </p>
          ) : null}
          {form.buildingId !== '' && createUnits.length > 0 && vacantCreateUnits.length === 0 ? (
            <p className="muted small admin-occupants-create-inline-hint">
      ...
      <section className="card">
        <div className="admin-occupants-list-header">
          ...
          <div className="admin-occupants-name-search-wrap">
            ...
            <input
              id="admin-occupants-name-search"
              type="search"
              className="mr-search-input admin-occupants-name-input"
                ...
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      ...
                      {r.profileStatus === 'declined' && r.adminComment ? (
                        <div className="occupant-admin-note muted small">Comment: {r.adminComment}</div>
                      ) : null}
                    </td>
                    ...
                    <td className="admin-occupants-actions">
                      ...
                      <button
                        ...
                        disabled={actionUserId === r.id || removeBusy}
                        onClick={() => setRemoveUser(r)}
                      >
                        Remove
                      </button>
                      {r.profileStatus === 'pending_review' ? (
                        <>
                          <button
                            type="button"
                            className="btn-secondary btn-small"
      ...
      {detailOpenForId != null ? (
        ...
        >
          <div className="modal-panel occupant-detail-modal">
            ...
            {detail ? (
              <dl className="occupant-detail-dl">
                <dt>Name</dt>
                <dd>{detail.name}</dd>
                <dt>Email</dt>
                <dd>{detail.email}</dd>
                ...
                <dd>
                  {detail.emergencyContactName?.trim() || '—'}
                  {detail.emergencyContactPhone?.trim() ? (
                    <span className="muted"> · {detail.emergencyContactPhone}</span>
                  ) : null}
                </dd>
                <dt>About</dt>
                <dd className="occupant-detail-about">{detail.aboutMe?.trim() || '—'}</dd>
                <dt>Admin comment</dt>
                <dd>{detail.adminComment?.trim() || '—'}</dd>
                <dt>Submitted</dt>
            ...
            <form onSubmit={(e) => void submitEdit(e)}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, buildingId: e.target.value, unitId: '' }))}
                  disabled={editSaving}
                >
                  <option value="">— None —</option>
                  {buildings.map((b) => (
              ...
              <label>
                ...
                <select
                  ...
                  disabled={editSaving || editForm.buildingId === ''}
                >
                  <option value="">— No unit —</option>
                  {selectableEditUnits.map((u) => (
                    <option key={u.id} value={u.id}>
                      Unit {u.unitCode}
                      {u.floor ? ` · floor ${u.floor}` : ''}
                    </option>
      ...
      {removeUser ? (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setRemoveUser(null)
          }}
        >
          <div className="modal-panel">
            <h3 id="remove-title">Remove occupant</h3>
            <p>
              Permanently delete <strong>{removeUser.name}</strong> ({removeUser.email})? Related notifications, bills,
              and maintenance records tied to this account may be removed by the database.
            </p>
            <div className="building-details-actions modal-footer-row">
      ...
      {declineModalUser ? (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setDeclineModalUser(null)
          }}
        >
          <div className="modal-panel building-form">
            <h3 id="decline-title">Decline profile</h3>
            <p className="muted">
              {declineModalUser.name} ({declineModalUser.email}) will be asked to update and resubmit. A comment is
              required.
            </p>
            <label>
            ...
            <div className="building-details-actions modal-footer-row">
              ...
              <button
                ...
                disabled={actionUserId === declineModalUser.id}
