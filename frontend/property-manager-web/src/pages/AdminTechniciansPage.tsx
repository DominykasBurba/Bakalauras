...
import { getAdminTechnicians } from '../services/api';
...
function DataCellList({ items, empty }: {
    ...
    return (<span className="admin-datatable-cell-list" title={text}>
      {text}
    </span>);
}
export function AdminTechniciansPage() {
    ...
    return (<div className="page admin-technicians-page">
      <h1>Technicians &amp; vendors</h1>

      {error && <p className="error">{error}</p>}

      <section className="card">
        <h3 className="card-title">Technicians &amp; vendors</h3>
        {loading ? (<p className="muted">Loading…</p>) : rows.length === 0 ? (<p className="muted">No technician accounts found.</p>) : (<div className="admin-technicians-table-wrap">
              ...
              <tbody>
                {rows.map((r) => (<tr key={r.userId}>
                    <td>
                      ...
                      {r.unitLabel?.trim() ? (<span className="muted small admin-technicians-unit"> · {r.unitLabel.trim()}</span>) : null}
                    </td>
                    ...
                    <td>
                      {(r.pendingOfferedReviewCount ?? 0) > 0 ? (<span className="admin-technicians-pending-offers" title="Self-listed services awaiting approval">
                    ...
                    <td className="admin-technicians-row-actions">
                      <Link to={`/admin/technicians/${r.userId}`} className="btn-link admin-technicians-open-link">
                        Open profile
                      </Link>
                    </td>
                  </tr>))}
