...
import { postAdminBroadcastNotification } from '../services/api';
...
export function AdminSendNotificationsPage() {
    ...
    useEffect(() => {
        if (selectedBuildingId != null)
            setTargetBuildingId(selectedBuildingId);
    }, [selectedBuildingId]);
    async function handleSubmit(e: FormEvent) {
        ...
        try {
            ...
            if (result.recipientCount === 0) {
                ...
                showToast(`Sent to ${result.recipientCount} resident${result.recipientCount === 1 ? '' : 's'}. They will see it under Notifications.`, 'success');
                setMessage('');
            }
        }
        catch (err) {
    ...
    return (<div className="page admin-send-notifications-page">
      ...
      <section className="card">
        ...
        <form className="building-form" onSubmit={(e) => void handleSubmit(e)}>
          <label>
            Audience
            <select value={targetBuildingId === null ? 'all' : String(targetBuildingId)} onChange={(e) => {
          ...
          <label>
            ...
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5} required maxLength={4000} placeholder="e.g. Water will be shut off Tuesday 9–11am for valve replacement."/>
          </label>
