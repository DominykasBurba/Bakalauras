...
import { getMaintenanceRequests, getNotifications, getScheduledMaintenanceForResident } from '../api'
...
export function ResidentDashboard() {
  ...
  const pendingReview =
    auth?.profileStatus === 'pending_review' ? (
      <section className="card profile-review-banner profile-review-pending">
        <p>
  ...
  return (
    <div className="page">
      ...
      <section className="card">
        ...
        ) : dashboardMaintenancePreview.length === 0 ? (
          <p className="muted">No requests yet — report an issue to get started.</p>
        ) : (
          <>
            <ul className="request-list">
              {dashboardMaintenancePreview.map((r) => (
            ...
            {hasMoreMaintenance && (
              <div className="request-list-see-all">
      ...
      <div className="grid-two">
        <section className="card">
          ...
          {loading ? (
            ...
            <p className="muted">No scheduled maintenance announcements for your building.</p>
          ) : (
            upcomingMaintenance.map((m) => (
              <div key={m.id} className="upcoming-item">
                ...
                {m.description?.trim() ? (
                  <p className="muted upcoming-maint-desc">{m.description.trim()}</p>
                ) : null}
