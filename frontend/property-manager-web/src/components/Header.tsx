...
function parseResidentLocation(unit: string | undefined): {
    building: string;
    unitLine: string;
} {
    if (!unit?.trim())
        return { building: '—', unitLine: '' };
      ...
      <div className="header-right">
        {isAdmin ? (<>
            <div className="building-select-wrapper" ref={dropdownRef}>
              <button type="button" className="building-select" onClick={() => setDropdownOpen((o) => !o)} disabled={loading}>
              ...
              {dropdownOpen && buildings.length > 0 && (<div className="building-dropdown">
                  <button type="button" className={`building-option ${selectedBuildingId == null ? 'selected' : ''}`} onClick={() => {
                  ...
                  {buildings.map((b) => (<button key={b.id} type="button" className={`building-option ${selectedBuildingId === b.id ? 'selected' : ''}`} onClick={() => {
            ...
            <div className="building-select building-select--readonly" aria-label="Your building and unit">
              <div className="building-info">
                <span className="building-name">{residentLoc.building}</span>
                {residentLoc.unitLine ? (<span className="building-units">{residentLoc.unitLine}</span>) : null}
              </div>
            </div>
          </div>)}
        <div className="header-user-actions">
          <Link to="/account" className="user-avatar user-avatar--link" title="Profile — account and password" aria-label="Open profile">
