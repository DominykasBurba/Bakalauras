...
const navItems: {
  ...
  /** Technician only (service provider dashboard & offered services). */
...
export function Sidebar() {
  ...
  const visibleNav = navItems.filter((item) => {
    if (item.adminOnly && !isAdminRole(auth?.role)) return false
    if (item.technicianOnly && !isTechnicianRole(auth?.role)) return false
    if (item.residentOnly && (isAdminRole(auth?.role) || isTechnicianRole(auth?.role))) return false
    return true
  })
        ...
        {visibleNav.map((item) => (
          <NavLink
            ...
            to={item.path}
              ...
              item.path === '/service-provider/offered-services'
            }
