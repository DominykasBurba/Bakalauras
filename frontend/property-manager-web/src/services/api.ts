import type { AssignOccupancyPayload, CreateOccupantPayload, EndOccupancyPayload, Bill, Building, BuildingInput, DashboardSummary, MaintenanceRequest, NotificationItem, PendingTechnicianOfferedServiceRow, OccupancyListItem, OccupantAdminDetail, OccupantAdminRow, UpdateOccupantPayload, PropertyOverview, ResidentPicker, ResidentProfilePayload, ResidentProfileResponse, ScheduledMaintenanceItem, ServiceCatalogItem, SessionResponse, TechnicianAssignmentContext, TechnicianDetail, TechnicianDirectoryRow, TechnicianNameOption, TechnicianOfferedService, TechnicianProfile, TechnicianProfileWritePayload, UnitDto, UnitWrite, } from '../types';
...
function messageFromApiErrorBody(body: string, status: number): string {
    const raw = body.trim();
    if (raw.startsWith('{')) {
        try {
...
async function request<T>(path: string, token?: string, init?: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
        ...
        throw new Error(messageFromApiErrorBody(body, response.status));
    }
...
export function getDashboardSummary(token: string, buildingId?: number | null) {
    const q = buildingId != null && buildingId !== undefined
...
export async function markNotificationUnread(token: string, notificationId: number): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/dashboard/notifications/${notificationId}/unread`, {
        method: 'PATCH',
        headers: {
            Authorization: `Bearer ${token}`,
        },
...
export function getMaintenanceRequest(token: string, requestId: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}`, token);
}
export function createMaintenanceRequest(token: string, title: string, description: string, priority: string, photoUrls?: string[]) {
    return request<MaintenanceRequest>('/maintenancerequests', token, {
        method: 'POST',
        body: JSON.stringify({
            title,
...
export function assignMaintenanceTechnician(token: string, requestId: string, assignedTechnician: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/technician`, token, {
        method: 'PATCH',
        body: JSON.stringify({ assignedTechnician }),
    });
}
export function patchMaintenancePriority(token: string, requestId: string, priority: 'Low' | 'Medium' | 'High') {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/priority`, token, {
        method: 'PATCH',
        body: JSON.stringify({ priority }),
    });
}
export function updateMaintenanceRequestStatus(token: string, requestId: string, status: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/status`, token, {
        method: 'PATCH',
        body: JSON.stringify({ status }),
    });
}
export function approveMaintenanceRequest(token: string, requestId: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/approve`, token, { method: 'POST' });
}
export function declineMaintenanceRequest(token: string, requestId: string, reason?: string | null) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/decline`, token, {
        method: 'POST',
        body: JSON.stringify({ reason: reason?.trim() || null }),
    });
}
export function completeMaintenanceWithoutCharge(token: string, requestId: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/complete-without-charge`, token, { method: 'POST' });
}
export function patchResidentCompletionFeedback(token: string, requestId: string, feedback: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/resident-feedback`, token, {
        method: 'PATCH',
        body: JSON.stringify({ feedback }),
    });
}
export function patchAdminResidentResponse(token: string, requestId: string, message: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/admin-resident-response`, token, {
        method: 'PATCH',
        body: JSON.stringify({ message }),
    });
}
export function patchTechnicianMaintenanceStatus(token: string, requestId: string, body: {
    status: string;
    completionNotes?: string;
}) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/technician-status`, token, {
        method: 'PATCH',
        body: JSON.stringify(body),
    });
}
export function patchTechnicianSiteDetails(token: string, requestId: string, body: {
    siteUpdate: string;
    materialsUsed: string;
    expectedReturnDate: string;
    officeNotes: string;
}) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/technician-site-details`, token, {
        method: 'PATCH',
        body: JSON.stringify(body),
    });
}
export function patchTechnicianInvoice(token: string, requestId: string, body: {
    invoiceUrl: string;
    amount?: number;
    notes?: string;
    lineItems?: Array<{
    ...
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/technician-invoice`, token, {
        method: 'PATCH',
        body: JSON.stringify(body),
    });
}
export function patchTechnicianPayout(token: string, requestId: string, body: {
    status?: string;
    approvedAmount?: number;
    paidAt?: string;
    notes?: string;
}) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/technician-payout`, token, {
        method: 'PATCH',
        body: JSON.stringify(body),
    });
}
export function postResidentChargeFromMaintenance(token: string, requestId: string, body: {
    amount: number;
    type?: string;
    dueDate?: string;
}) {
    ...
    }>(`/maintenancerequests/${encodeURIComponent(requestId)}/resident-charge`, token, {
        method: 'POST',
        body: JSON.stringify(body),
    });
}
export function patchResidentChargeFromMaintenance(token: string, requestId: string, body: {
    amount: number;
    type?: string;
    dueDate?: string;
}) {
...
export function postResidentChargeSendToResident(token: string, requestId: string) {
    return request<MaintenanceRequest>(`/maintenancerequests/${encodeURIComponent(requestId)}/resident-charge/send`, token, { method: 'POST', body: '{}' });
}
...
export function assignOccupancy(token: string, unitId: number, body: AssignOccupancyPayload) {
    ...
    if (body.leaseEndDate?.trim())
        payload.leaseEndDate = body.leaseEndDate.trim();
...
export function createBillingCheckoutSession(token: string, billId: string) {
    return request<{
        url: string;
    }>('/billing/checkout-session', token, {
        method: 'POST',
...
export function createAdminScheduledMaintenance(token: string, body: {
    buildingId: number;
    title: string;
    ...
    return request<{
        id: number;
    }>('/admin/scheduled-maintenance', token, {
        method: 'POST',
        body: JSON.stringify(body),
    });
}
export function updateAdminScheduledMaintenance(token: string, id: number, body: {
    buildingId: number;
    title: string;
    description?: string | null;
    scheduledDate: string;
...
export function postAdminBroadcastNotification(token: string, body: {
    message: string;
    ...
    return request<{
        ...
        warning?: string;
    }>('/admin/notifications/broadcast', token, {
        method: 'POST',
        body: JSON.stringify(body),
    });
}
...
export function createTechnicianOfferedService(token: string, body: {
    title: string;
    description?: string | null;
    sortOrder?: number;
}) {
...
export function changePassword(token: string, body: {
...
export function getAdminOccupants(token: string, params?: {
    buildingId?: number | null;
    name?: string | null;
}) {
...
export function approveOccupantProfile(token: string, userId: number) {
    ...
    }>(`/admin/occupants/${userId}/approve`, token, {
        method: 'POST',
    });
}
export function declineOccupantProfile(token: string, userId: number, comment: string) {
    ...
    }>(`/admin/occupants/${userId}/decline`, token, {
        method: 'POST',
...
export function updateAdminOccupant(token: string, userId: number, body: UpdateOccupantPayload) {
    ...
    return request<{
        ...
        email: string;
        buildingId?: number | null;
    }>(`/admin/occupants/${userId}`, token, { method: 'PUT', body: JSON.stringify(payload) });
}
...
export function getAdminTechnicianNames(token: string, catalogItemId?: number | null) {
    const q = catalogItemId != null && catalogItemId > 0
...
export function postAdminServiceCatalogItem(token: string, body: {
    name: string;
    description?: string | null;
    sortOrder?: number;
}) {
...
export function putAdminServiceCatalogItem(token: string, id: number, body: {
    name: string;
    description?: string | null;
    sortOrder?: number;
}) {
...
export function getAdminPendingTechnicianOfferedServices(token: string) {
    return request<PendingTechnicianOfferedServiceRow[]>('/admin/offered-services/pending-review', token);
}
...
export function putAdminTechnicianOfferedServiceReview(token: string, userId: number, offeredServiceId: number, body: OfferedServiceAdminReviewBody) {
    return request<TechnicianOfferedService>(`/admin/technicians/${userId}/offered-services/${offeredServiceId}/review`, token, {
        method: 'PUT',
        body: JSON.stringify({
...
export function putAdminTechnicianProfile(token: string, userId: number, body: TechnicianProfileWritePayload) {
